...
- `cutout`：优先识别模板中的绿布占位区，使用 `rembg` 自动抠出商品并按绿布尺寸等比嵌入；如果模板没有绿布，则回退到真实场景中的居中主体合成。
...
3. 抠图模式会按照商品主体的 alpha 边界等比缩放，并生成模糊接触阴影和环境阴影，减少“贴纸感”。检测到绿布时只替换绿布区域，模板中的 Logo、标题和规则文案保持原始清晰度。
