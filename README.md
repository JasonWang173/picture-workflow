...
- `cutout`：优先识别模板中的绿布占位区，直接将商品原图按绿布尺寸等比嵌入，不对商品图做模糊或二次主体叠加；如果模板没有绿布，则使用 `rembg` 自动抠出商品并回退到真实场景中的居中主体合成。
...
3. 绿布模板只替换绿布区域，商品原图使用等比例高质量重采样，不变形、不额外模糊；模板中的 Logo、标题和规则文案保持原始清晰度。没有绿布的模板才按照商品主体的 alpha 边界抠图，并生成接触阴影。
